...
def setup_training(self):
    ...
    self.gamma = 0.6
    r = 4
    matrix = coo_matrix((data, (row, col)), shape=(18*10**17 +1, 6),dtype = np.float64)
    q_values = matrix.todok()
    #q_values = np.ones((201411,6))
    #q_values[:, 5] = -40000
    #q_values[:, 4] = -40000
    self.values = np.array([])
    if self.train and os.path.isfile("q_values.npz"):
        #np_load_old = np.load
...
def symmetry_up_down(features):
    ...
    return (d, u, l, r,m1x, -m1y,free_spaces_complete, bomb1x, -bomb1y,bomb_timer1)
...
def symmetry_left_right(features):
    ...
    return (u, d, r, l,-m1x, m1y,free_spaces_complete, -bomb1x, bomb1y,bomb_timer1)
...
def symmetry_x_y(features):
    ...
    return (l, r, u, d,m1y, m1x,free_spaces_complete, bomb1y, bomb1x, bomb_timer1)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if self.rounds%250 ==0:
        ...
        sparse.save_npz("q_values.npz", matrix)
